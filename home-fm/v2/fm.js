...
async function handleFile(filePath, stats, mediaData) {
    ...
    if (stats.isFile()) {
        if (['.mp4', '.mkv', '.avi'].includes(fileExtension)) {
            ...
            const result = await searchMediaInfo(fileName);
            if (result) {
                movieData.tmdbInfo = result;
            }
